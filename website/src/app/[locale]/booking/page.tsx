...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .order('sort_order')
      .then(({ data }) => {
        const list = (data as Package[]) || [];
        setPackages(list);
        if (list.length) setSelectedPkg(list[0].id);
      });
        ...
        <aside className="h-fit rounded-owj border border-black/5 bg-surmei p-6 text-white shadow-owj lg:sticky lg:top-24">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            ...
            <div className="flex justify-between gap-3">
              ...
              <dd className="font-bold">{selectedSlot ? fmtSlot(slots.find((s) => s.id === selectedSlot)!.start_at) : '—'}</dd>
            </div>
